...
```{r Exercise 1.7}
# write code here
target_rows <- rownames(mtcars) %in% c("Duster 360", "Lincoln Continental", "Maserati Bora")
hp_col <- which(colnames(mtcars) == "hp")
mtcars[target_rows, hp_col]
```
...
```{r Exercise 1.11}
# write code here
am_idx <- which(colnames(mtcars) == "am")
mtcars_no_am <- mtcars[, -am_idx]
```
...
```{r}
write.csv(clinic, "../analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("../analysis_data/brca_clinical_data.csv")
```
...
4. around 55 years now - around 10 years = 45 years old
...
```{r Exercise 4.1}
follow_up <- as.numeric(clinic$last_contact_days_to)
mask <- !is.na(clinic$age_at_diagnosis) & !is.na(follow_up)

plot(clinic$age_at_diagnosis[mask],
     follow_up[mask],
...
# create plot here
## ---- Exercise 4.2 ----
# convert to numeric / factor as needed
nodes <- as.numeric(clinic$lymph_nodes_examined_count)
dx_year <- as.factor(clinic$initial_pathologic_dx_year)

# drop rows with missing or non-numeric values
mask <- !is.na(nodes) & !is.na(dx_year)

boxplot(nodes[mask] ~ dx_year[mask],
        main = "Lymph Nodes Examined by Diagnosis Year",
...
```{r}
death_col <- intersect(c("death_days_to","days_to_death"), colnames(cleaned_clinic))[1]
alive_col <- intersect(c("last_contact_days_to","days_to_last_follow_up","days_to_last_followup"),
                       colnames(cleaned_clinic))[1]

to_num <- function(x) suppressWarnings(as.numeric(x))

death_num <- if (!is.na(death_col)) to_num(cleaned_clinic[[death_col]]) else rep(NA_real_, nrow(cleaned_clinic))
alive_num <- if (!is.na(alive_col)) to_num(cleaned_clinic[[alive_col]]) else rep(NA_real_, nrow(cleaned_clinic))

# if death time is missing/nonpositive, use last contact; otherwise use death time
cleaned_clinic$survival_time <- ifelse(is.na(death_num) | death_num <= 0, alive_num, death_num)
```
...
The probability of surviving (the estimated proportion of patients still alive) at a given time point.

# 2. What does the x axis represent?
Time since diagnosis (in days) until either death or last follow-up.

# 3. What age group appears to have the greatest rate of survival?
The Middle age group (orange line) maintains the highest survival curve over time.

# 4. What age group appears to have the worst rate of survival? 
The Old age group (green line) shows the lowest survival probabilities, especially after several thousand days.

# 5. What is the approx. 15-year survival rate for "Old" breast cancer patients?
15 years ≈ 15 × 365 ≈ 5,500 days.
At about 5,500 days on the x-axis, the green curve is roughly around ~50% survival.
...
The p-value is 0.21.
This is not statistically significant (it’s well above the common 0.05 threshold), indicating that there is no strong evidence of a difference in survival between the age categories in this dataset.
