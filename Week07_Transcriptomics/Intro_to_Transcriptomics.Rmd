...
## 1) Auto-detect clinical ID column by overlap with counts (16-char or 12-char) ----
...
## 2) Collapse counts to unique sample keys (handles multiple aliquots per sample) ----
...
# keep counts for retained samples; integers for DESeq2
...
```{r}
# ---- Problem 3 (full-rank safe): choose 2 groups, build design, auto-simplify if not full rank ----
suppressPackageStartupMessages({ library(DESeq2) })

stopifnot(exists("rna_counts_filt"), exists("colData_df"), exists("def_col"), exists("rna_genes"))

## 0) Sync samples between counts and colData
...
## 1) Choose a usable 2-group variable: definition -> PAM50 (top2) -> age median split
...
## 2) Build covariates (drop constants), then drop rows with NA in any design term
...
## 3) Try full design; if not full rank, fall back to ~ group only
...
if (inherits(attempt, "try-error")) {
  # if it failed before, force minimal model
  message("Falling back to minimal design: ~ ", grp_col)
  attempt <- make_dds(grp_col)
}

# Now try DESeq(); if full-rank error occurs, simplify to ~ group
...
```
```{r}
# ---- Problem 4: install/load EnhancedVolcano and plot ----
...
# Optional: save the figure next to your Rmd
# ggsave("volcano_contrast.png", p, width = 8, height = 6, dpi = 300)
